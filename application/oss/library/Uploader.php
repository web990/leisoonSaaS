...
class Uploader
{
     ...
     */
    public function __construct($fileField,$type = "upload", $config=null)
    {
        $this->fileField = $fileField;
        $this->config = $config ? $config:config('upload.'.$type);
        $this->type = $type;
        $this->rootPath = isset($this->config['root_path']) ? $this->config['root_path']:'';
    }

    public function upload(){
        $file = request()->file($this->fileField);
        if ($file){
            $info = $file->move($this->rootPath, true, false);
            if ($info){
    ...
    protected function parseFile($info) {
        ...
        $data['name']        = $data['file']['name']; //获取原始文件名

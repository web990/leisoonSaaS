...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        $user_groups = Db::name($this->config['auth_group_access'])
            ->alias('aga')
            ->join('__' . strtoupper($this->config['auth_group']) . '__ ag', 'aga.role_id = ag.id', 'LEFT')
            ->field('aga.uid,aga.role_id,ag.id,ag.pid,ag.name,ag.rules')
    ...
    public function getRuleList($uid)
    {
        ...
        if (2 == $this->config['auth_type'] && Cache::has('_rule_list_' . $uid))
        {
            return Cache::get('_rule_list_' . $uid);
        }
        ...
        {
            //规则列表结果保存到Cache
            Cache::set('_rule_list_' . $uid, $rulelist);
        }
